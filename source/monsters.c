    ...
    if (cur->health <= 0) {
        ...
        if (cur->type == KAMIKAZE) {
            ...
            for (int j = 0; j < w->towerNumber; j++) {
                s8 dx = abs(f32togrid(w->towers[j].pos.x) - f32togrid(cur->pos.x));
                s8 dy = abs(f32togrid(w->towers[j].pos.y) + (w->towers[j].screen == MAIN_SCREEN ? 0 : 12) - f32togrid(cur->pos.y) - (cur->screen == MAIN_SCREEN ? 0 : 12));
                if (MAX(dx, dy) <= 2) {
                    w->players[cur->player].money += 2;
                    deleteTower(w, j);
                    j--;
